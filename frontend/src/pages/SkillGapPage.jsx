...
export const SkillGapPage = ({ onLaunchPractice }) => {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-8 animate-fade-in">
      ...
      {/* Executive Briefing Card for Skill Gap (when Make it brief is ON) */}
      {isBriefMode && (
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-amber-500/20 pb-3">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-base font-black text-slate-900 dark:text-white flex items-center gap-2">
                  <span>{t('briefSkillGapTitle')}</span>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  {t('targetCareerTrack')} <span className="font-semibold text-brand-600 dark:text-brand-400">{t(selectedRole)}</span>
                </p>
              </div>
            </div>
            ...
            <button
              onClick={() => {
                ...
                const missingList = gapData?.missing_skills?.slice(0, 3).map(s => t(s.name)).join(', ') || '';
                const bullets = [
                  t('briefSkillGapTitle'),
                  t('briefSkillGapBullet1', { role: t(selectedRole) }),
                  t('briefReadinessBullet', {
                    pct: gapData?.readiness_percentage || 0,
                    count: gapData?.acquired_skills?.length || 0,
                    remaining: gapData?.missing_skills?.length || 0
                  }),
                  t('briefSkillGapBullet2'),
                  missingList ? `${t('currentGaps')}: ${missingList}` : null
                ].filter(Boolean);
                speak(bullets.join('. '));
              }}
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <button
              onClick={() => toggleSpeak(`${t('skillGapEngine')}. ${t('skillGapSubtitle')}. ${t('targetCareerTrack')} ${t(selectedRole)}.`)}
              title={t('readAloud')}
